...
function ts_makeSortable(t) {
    ...
    for (var i = 0; i < firstRow.cells.length; i++) {
        ...
        // [***] tady se nevim proc osekavaly html tagy, coz mi komplikovalo javascriptovy zobrazovani kurzu - upravil jsem to na innerHTML, pouziva se to jen v prehledu kurzu a tam to funguje [***]
        //var txt = ts_getInnerText(cell);
        var txt = cell.innerHTML;
        if (cell.className != "unsortable" && cell.className.indexOf("unsortable") == -1) {
            if (i != firstSort['column']) {
                cell.innerHTML = '<a href="#" class="sortheader inactive-' + defaultSort + '" onclick="ts_resortTable(this, '+i+');return false;">'+txt+'<span class="sortarrow" sortdir="' + defaultSort + '"></span></a>';
            } else {
                cell.innerHTML = '<a href="#" class="sortheader active-' + firstSort['sort-dir'] + '" onclick="ts_resortTable(this, '+i+');return false;">'+txt+'<span class="sortarrow" sortdir="' + (firstSort['sort-dir'] == 'down' ? 'up' : 'down') + '"></span></a>';
            }
        }
    }
...
function ts_resortTable(lnk, clid) {
        ...
        if (tr.childNodes[i].tagName && tr.childNodes[i].tagName.toLowerCase() == 'th') {
            ...
            for (var j = 0; j < thNodes; j++) {
                if (th.childNodes[j].tagName && th.childNodes[j].tagName.toLowerCase() == 'a') {
                    var active = false;
                    if (ts_hasClassName(th.childNodes[j], activeDown)) {
                        th.childNodes[j].className = th.childNodes[j].className.replace(activeDown, inactiveDown);
                        active = true;
                    } else if (ts_hasClassName(th.childNodes[j], activeUp)) {
    ...
    // Set active sort
    if (ts_hasClassName(lnk, inactiveDown)) {
        lnk.className = lnk.className.replace(inactiveDown, activeDown);
    } else if (ts_hasClassName(lnk, inactiveUp)) {
    ...
    if (span.getAttribute("sortdir") == 'down') {
        ...
        if (ts_hasClassName(lnk, activeUp)) {
            ts_removeClassName(lnk, activeUp);
            ts_addClassName(lnk, activeDown);
        }
    } else {
        ARROW = '';
        span.setAttribute('sortdir','down');
        // change direction class
        if (ts_hasClassName(lnk, activeDown)) {
                ts_removeClassName(lnk, activeDown);
                ts_addClassName(lnk, activeUp);
        }
    }
...
function ts_addClassName(e, newClassName)
{
    e.className = e.className + ' ' + newClassName;
}

function ts_hasClassName(e, findClassName)
{
...
function ts_removeClassName(e, findClassName)
{
    var classes = e.className.split(' ');
    var cl = classes.length;
    ...
    for (var i = 0; i < cl; i++) {
        if (findClassName == classes[i]) {
            e.className = e.className.replace(findClassName, '');
        }
    }
}
...
function clean_num(str) {
    if (str != undefined) str = str.replace(new RegExp(/[^-?0-9.]/g),"");
    return str;
}
...
//		var txt = ts_getInnerText(cell);
//		if (cell.className != "unsortable" && cell.className.indexOf("unsortable") == -1) {
//			cell.innerHTML = '<a href="#" class="sortheader" onclick="ts_resortTable(this, '+i+');return false;">'+txt+'<span class="sortarrow"'+(cell.className.indexOf("defaultdesc") != -1 ? ' sortdir="down"' : '')+'><img src="'+ image_path + image_none + '" alt="&darr;"/></span></a>';
//		}
//	}
//	if (alternate_row_colors) {
//		alternate(t);
